...
class Comment extends Model
{
    ...
    public static function get_comment_child($parent_id, $page_show = 3){
        ...
        for ($i=0; $i < count($comment_child); $i++) { 
            $comment_child[$i]->comment_like_count = Like::where('reference_id', $comment[$i]->id)->where('table_name', 'comments')->count();
            $comment_child[$i]->comment_liked_by_me = Like::where('reference_id', $comment[$i]->id)->where('user_id', Auth::id())->where('table_name', 'comments')->count();    
        }
